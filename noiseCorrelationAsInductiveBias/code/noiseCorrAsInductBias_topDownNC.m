...
% Can top-down message passing promote useful noise correlations to speed
% learning? 

% Goals: 
%        1) test whether symmetrical top-down feedback projections can
%        promote formation of noise correlations in the signal dimension
%        2) test whether this can speed up learning


% Next steps:

% run simulation with various levels of: numberOfSignalsToInjectPerUnit
% Test learning speed of different models and look at covariance of the
% firing rates stored in the output structure. Does more time lead to more
% noise correlations? What about faster learning? 
...
input.doOpt            = true
input.all_fracCorrNoise= 0;
% MRN added this!
input.numberOfSignalsToInjectPerUnit = 2;
...
for i = 1:numReps
    ...
    output=runCorrLearningSim_tdnc(input)
...
%% does hebbian learning improve accuracy:

clear weightDiff
if doRL
weightDiff=squeeze(finalWeights2(:,1,:,:)- finalWeights2(:,2,:,:));
normWeightDiff=weightDiff./repmat(std(weightDiff), size(weightDiff, 1), 1) ;
else
weightDiff=finalWeights2   
end
...
getCbColors
for i = 1:length(hebbLRs)
    a(i)=plot(weightDiff(:,i), 'color', cbColors(i+1,:));
end
ff=legend(a, 'No Hebb', 'Hebbian')
ylabel('weight')
xlabel('neuron')
...
figure
toShow=[1,2];
hist([normWeightDiff(1:100,toShow(1)), normWeightDiff(101:200,toShow(1))], -2:.2:2)
b=get(gca, 'Children')
...
ylabel('Frequency')
xlabel('Weights')
title('Without Hebbian')


figure

toShow=[1,2];
hist([normWeightDiff(1:100,toShow(2)), normWeightDiff(101:200,toShow(2))], -2:.2:2)
b=get(gca, 'Children')
set(b(1), 'faceColor', cbColors(5,:))
set(b(2), 'faceColor', cbColors(6,:))
xlim([-3, 3])
...
% May 1
% save MRN_messingAroundWorkspace.mat
