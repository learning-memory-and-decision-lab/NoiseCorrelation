function [noiseCorrAdvantage, SNR]= get_noiseCorrAdvantage(input)
   %keyboard
   % Params
   ...
   mu        = input.mu*sqrt(var_sig);% Amount of signal in each trial
   n         = input.n; %;              % Total number of neurons
   phi_list  = [0, .2];         % Set of phi values to investigate
   ...
   noiseCorrAdvantage=sum(acc(:,end)-acc(:,1));
