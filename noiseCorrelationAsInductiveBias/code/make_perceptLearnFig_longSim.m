...
for xx = 1:length(axs)
    ...
    elseif xx==2
        ...
        %a=plot(optWeights, '--k');
        for i = 1:(nCorrs)
            plot(log(1:length(smMeanAcc(:,i))), smMeanAcc(:,i), 'color', ones(3,1).*i./(nCorrs+3));
...
fn=fullfile(pwd, 'NC_Fig2_longSim.eps');
%set(kk, 'string', 'Nassar et al 2018 Figure 1', 'position', [0.85 0.95 0.15 0.05], 'EdgeColor', 'none')
saveas(gcf,  fn, 'epsc2')
close(gcf)

fn=sprintf('NC_perceptLearnWorkspce_longSim_%s',  date )
save(fn)
