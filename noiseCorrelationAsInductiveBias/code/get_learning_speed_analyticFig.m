function [] = get_learning_speed_analyticFig()
   ...
   phi_list  = 0:0.02:.2;         % Set of phi values to investigate
   alpha     = 0.0001;           % Learning rate
   t_end     = 100000;              % Number of trials in simulation
   delta     = 0.5;              % Reward prediction error strength

   
   
   % Change population size n? change pools -- keep signal to noise
   % More time t_end
   % look at different learning rates (alpha)
   % spectrum of models from fixed signal-to-noise to fixed single unit
   % signal and variance)
   % all dim noise correlations
   ...
   % Plot the results
   plot_all(phi_list, tot_var_a, tot_var_e, dp, d, sig, acc, phi_sel, alpha, delta, mu, sig_star)
...
function [S, a] = get_cov_mat(rowsum, phi, n)
   ...
   S = phi*a*m1*m1' + phi*a*m2*m2' + (1-phi)*a*eye(200);
...
function [] = plot_all(phi_list, tot_var_a, tot_var_e, dp, d, sig, acc, phi_sel, alpha, delta, mu, sig_star)
...
%    legend(namelist)
   ...
   grays=repmat((0: .9./length(phi_list) : .9)', 1, 3)
   
   figure
   hold on
   for i = 1:length(phi_list);
      a=plot(log10(1:length(acc)),  acc(:,i), 'color', grays(i,:), 'lineWidth', 2);  
   end
   grid on
   title('Analytic Accuracy')
   xlabel('Time (log units)')
   ylabel('Percent Correct')
   %legend([a(1), a(end)],  'Location', 'SouthEast')
   saveas(gcf, 'analyticAccuracyPlot_asymptote.eps', 'epsc2')
