function output=runCorrLearningSim_tdnc(input)
...
% MRN added this!
numberOfSignalsToInjectPerUnit=input.numberOfSignalsToInjectPerUnit
...
for k = 1:length(all_fracCorrNoise)
    ...
    % frVar = totVar ./ (effectiveUnits + effectiveUnits.*(effectiveUnits-1).*fracCorrNoise);
    
    % MRN added this so that Linghai and Niloufar could control signal to
    % noise across multiple timesteps (ie. numberOfSignalsToInjectPerUnit):
    
    effectiveUnits = nNeuronsPerPool.*numberOfSignalsToInjectPerUnit
...
%     end
    ...
    firingRates=nan(length(Mu), nTrials);
    
    
    for i = 1:nTrials
        
        FR=zeros(size(Mu)); % MRN;
        Mu(poolID==allStims(i))=targFR;
        Mu(poolID~=allStims(i))=nonTargFR;
        decNeuronOutput=zeros(1, 2); % MRN
        
        
        % Loop through timesteps within a single trial
        for ii = 1:numberOfSignalsToInjectPerUnit
            FR=mvnrnd(Mu,covMat)+decNeuronOutput*wtMatrix'; % pass back decision activity to input units
