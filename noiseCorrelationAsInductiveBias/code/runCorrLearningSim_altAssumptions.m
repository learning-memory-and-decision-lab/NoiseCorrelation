function output=runCorrLearningSim_altAssumptions(input)
    ...
    for j = 1:length(assumpParam)
       AP=assumpParam(j);
        ...
        if ~fixNoise
            ...
            % AP = 1 means use our methods
            % AP = 0 means use standard assumptions in the literature
            % Anything between those numbers is a weighted mixture. 
            
            targFR           = input.targFR.*scaleFactor.*AP + input.targFR.*(1-AP);
            nonTargFR        = input.nonTargFR.*scaleFactor.*AP + input.nonTargFR.*(1-AP);
          end
        ...
        for i = 1:nTrials
            ...
            accuracy(i,k,j)=choice(i)==allStims(i);
            
            if doOpt
                if ~doRL
                    optAccuracy(i,k,j)=sign(optOutput)==sign(targetOutputs(allStims(i)));
                else
                    [val,ind]=max(optOutput);
                    optAccuracy(i,k,j)=allStims(i)==ind;
                end
            end
        end
